...
function renderCourses(filterDept = 'all') {
    const courseList = document.getElementById('courseList');
    courseList.innerHTML = '';

    const filteredCourses = filterDept === 'all' 
        ? coursesData.courses 
        : coursesData.courses.filter(c => c.department === filterDept);

    filteredCourses.forEach(course => {
        const isInSchedule = schedule.some(c => c.id === course.id);
        const card = document.createElement('div');
        card.className = 'course-card' + (isInSchedule ? ' selected' : '');
        card.innerHTML = `
...
function openCourseModal(course) {
    ...
    document.getElementById('modalCredits').textContent = `${course.credits} Credits`;
    ...
    const prereqText = course.prerequisites.length > 0 
        ...
        : 'None';
    document.getElementById('modalPrerequisites').textContent = prereqText;
    
    const termsContainer = document.getElementById('modalTerms');
...
function addToSchedule() {
    if (currentCourse && !schedule.some(c => c.id === currentCourse.id)) {
        schedule.push(currentCourse);
        updateScheduleDisplay();
        renderCourses(document.getElementById('departmentFilter').value);
        closeModal();
    }
}

function removeFromSchedule(courseId) {
    schedule = schedule.filter(c => c.id !== courseId);
    updateScheduleDisplay();
    renderCourses(document.getElementById('departmentFilter').value);
}
...
function updateScheduleDisplay() {
    ...
    if (schedule.length === 0) {
        scheduleList.innerHTML = '<div class="empty-schedule">No courses added yet</div>';
    } else {
        schedule.forEach(course => {
            ...
            item.innerHTML = `
                <div class="schedule-item-info">
                    ...
                    <div style="font-size: 0.9em; color: #666;">${course.credits} Credits</div>
                </div>
...
function setupEventListeners() {
    ...
    document.getElementById('addCourseBtn').addEventListener('click', addToSchedule);
